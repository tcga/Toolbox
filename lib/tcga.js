...
(function (exports) {
    ...
    exports.TCGA = {
        ...
        loadScript: function (uri, callback, reload) {
            var semaphoreCallback, pendingScripts, errors = [], that = this;
        ...
        get: function (uri, callback) {
            ...
            semaphoreCallback = function (error) {
                ...
                for ( i = 0 ; i < uri.length ; i++ ){
                    if ( !datas[uri[i]] ) return;
                }
                if ( errors.length === 0 ) {
                  if (datas.length === 1) datas = datas[uri[0]];
                  callback(null, datas);
                }
                else {
                  callback(errors);
            ...
            uri.forEach(function(oneUri){
                $.ajax({
                  url : oneUri,
                  success : function (data) { datas[oneUri] = data; semaphoreCallback(null); },
