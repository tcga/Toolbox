...
(function (exports) {
 ...
 // The CORS-proxy.
    proxy = "https://aproxas.herokuapp.com/?url=";
 ...
 // @param {string} callback.body
    ...
    get.barcodes = function (uuids, callback) {
        if (typeof callback !== "function") {
            throw new Error("Please provide a callback parameter (function).");
        }
        ...
        $.ajax({
            url: proxy + encodeURIComponent("https://tcga-data.nci.nih.gov/uuid/uuidws/mapping/json/uuid/batch"),
            type: "POST",
            data: uuids.join(","),
            headers: {
            ...
            callback(null, data.uuidMapping);
        }).fail(function (jqXHR, textStatus, errorThrown) {
            if (errorThrown === "") {
                callback(new Error("Getting the URI failed. The browser log might have more details."), null);
            } else {
    ...
    get.uuids = function (barcodes, callback) {
        if (typeof callback !== "function") {
            throw new Error("Please provide a callback parameter (function).");
        }
        if (Array.isArray(barcodes) === false) {
            barcodes = [barcodes];
        }
        $.ajax({
            url: proxy + encodeURIComponent("https://tcga-data.nci.nih.gov/uuid/uuidws/mapping/json/barcode/batch"),
            type: "POST",
            data: barcodes.join(","),
            headers: {
                "Content-Type": "text/plain"
 ...
 // @param {object} callback.res SPARQL query results in the SPARQL 1.1 Query Results JSON Format.
    exports.find = find = function (query, callback) {
        ...
        TCGA.get.sparql(endpoint + encodeURIComponent(query), {
            useProxy: false
        }, callback);
    };
